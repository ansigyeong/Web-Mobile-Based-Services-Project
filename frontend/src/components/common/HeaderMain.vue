 ...
 <div  style="height:123px; position:absolute; z-index:100; " fixed="">
     ...
     <v-list  flat  class="primary--text" >
        ...
        <template v-if="this.$store.state.islogin"> 
          ...
          <v-list-item 
          @click="movePage(record.path)"
        >
        <v-list-item-icon><v-icon  v-text="record.icon"></v-icon></v-list-item-icon>
        <v-list-item-title v-text="record.title"></v-list-item-title>
...
export default {
    name: 'HeaderMain',
        ...
        data: () => ({
            ...
            drawer: false,
            keyword: null,
              record : {path : '/record' , title : '내 등급' , icon: 'fas fa-medal'},
              fame:   { path: '/fame', title: '명예의 전당', icon: 'fas fa-trophy' },
              bookmark:    { path: '/bookmark', title: '찜 목록', icon: 'fas fa-paperclip' },
